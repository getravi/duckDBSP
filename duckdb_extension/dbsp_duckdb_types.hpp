...
struct TableSchema {
    ...
    duckdb::LogicalType GetColumnType(size_t idx) const {
        return idx < columns.size() ? columns[idx].type : duckdb::LogicalType::VARCHAR;
    }

    std::string GetColumnName(size_t idx) const {
        return idx < columns.size() ? columns[idx].name : "col" + std::to_string(idx);
    }
};
...
private:
    void record_change(ChangeRecord::Type type, const DuckDBRow& row, const DuckDBRow& old_row) {
        ChangeRecord rec;
        rec.type = type;
        rec.row = row;
        rec.old_row = old_row;
...
public:
    explicit NativeMaterializedView(const std::string& name, const std::string& sql)
    ...
    virtual void apply_changes(const std::string& table_name, const DuckDBZSet& changes) = 0;
...
public:
    ...
    NativeFilterView(const std::string& name, const std::string& sql,
        ...
        : NativeMaterializedView(name, sql)
    ...
    void apply_changes(const std::string& table_name, const DuckDBZSet& changes) override {
    ...
    std::vector<std::string> source_tables() const override { return {source_table_}; }
...
public:
    ...
    NativeProjectView(const std::string& name, const std::string& sql,
                      const std::string& source_table, const TableSchema& result_schema,
                      ProjectFn project)
        : NativeMaterializedView(name, sql)
        , source_table_(source_table)
        , schema_(result_schema)
...
private:
    ...
    int64_t compute_agg(const AggState& state) const {
        switch (agg_type_) {
            case AggType::SUM: return state.sum;
            case AggType::COUNT: return state.count;
            case AggType::AVG: return state.count > 0 ? state.sum / state.count : 0;
...
public:
    ...
    const DuckDBZSet& get_result() const override { return result_; }
    const TableSchema& result_schema() const override { return schema_; }
    std::vector<std::string> source_tables() const override { return {left_table_, right_table_}; }
...
private:
    void add_join_result(const DuckDBRow& left, const DuckDBRow& right, Weight weight) {
        DuckDBRow joined;
    ...
    KeyFn left_key_;
    KeyFn right_key_;
    std::unordered_map<duckdb::Value, DuckDBZSet, duckdb::ValueHashFunction> left_indexed_;
    std::unordered_map<duckdb::Value, DuckDBZSet, duckdb::ValueHashFunction> right_indexed_;
    DuckDBZSet result_;
};
