...
public:
    ...
    ParseResult parse(const std::string& sql, const std::string& view_name = "") {
        ...
        try {
            ...
            if (stmt->type == duckdb::StatementType::SELECT_STATEMENT) {
                auto& select = stmt->Cast<duckdb::SelectStatement>();
                return parse_select(select, view_name.empty() ? "unnamed_view" : view_name);
            }

            // Check for CREATE VIEW
            if (stmt->type == duckdb::StatementType::CREATE_STATEMENT) {
                // Extract the view info from CREATE statement
                result.error = "Use SELECT syntax directly, CREATE VIEW parsing coming soon";
                return result;
            }
    ...
    ParseResult parse_select(const duckdb::SelectStatement& stmt, const std::string& view_name) {
        ParseResult result;
        ...
        auto& select = node->Cast<duckdb::SelectNode>();
        ...
        if (!parse_from_clause(select.from_table.get(), result.view_def)) {
            result.error = "Could not parse FROM clause";
            return result;
        }
...
private:
    bool parse_from_clause(duckdb::TableRef* ref, ParsedViewDef& def) {
        ...
        switch (ref->type) {
            ...
            case duckdb::TableReferenceType::JOIN: {
                ...
                if (!parse_from_clause(join.left.get(), def)) return false;
                if (!parse_from_clause(join.right.get(), def)) return false;
    ...
    void parse_join_condition(duckdb::ParsedExpression* expr, ParsedViewDef::JoinInfo& info) {
        if (expr->type == duckdb::ExpressionType::COMPARE_EQUAL) {
            auto& cmp = expr->Cast<duckdb::ComparisonExpression>();
            ...
            if (cmp.left->type == duckdb::ExpressionType::COLUMN_REF &&
                ...
                auto& left_col = cmp.left->Cast<duckdb::ColumnRefExpression>();
                auto& right_col = cmp.right->Cast<duckdb::ColumnRefExpression>();
    ...
    void parse_select_list(const std::vector<std::unique_ptr<duckdb::ParsedExpression>>& select_list,
                           ParsedViewDef& def) {
        for (size_t i = 0; i < select_list.size(); i++) {
            ...
            switch (expr->type) {
                ...
                case duckdb::ExpressionType::COLUMN_REF: {
                    auto& col = expr->Cast<duckdb::ColumnRefExpression>();
                    def.project_column_names.push_back(col.GetColumnName());
                    def.project_columns.push_back(i);  // Will be resolved later
                    break;
                }
                ...
                case duckdb::ExpressionType::FUNCTION: {
                    ...
                    if (func_name == "SUM" || func_name == "COUNT" ||
                        ...
                        if (!func.children.empty() &&
                            ...
                            auto& col = func.children[0]->Cast<duckdb::ColumnRefExpression>();
    ...
    void parse_where_clause(duckdb::ParsedExpression* expr, ParsedViewDef& def) {
        ...
        switch (expr->type) {
            ...
            case duckdb::ExpressionType::CONJUNCTION_AND: {
                auto& conj = expr->Cast<duckdb::ConjunctionExpression>();
    ...
    std::string get_comparison_op(duckdb::ExpressionType type) {
        switch (type) {
            case duckdb::ExpressionType::COMPARE_EQUAL: return "=";
            case duckdb::ExpressionType::COMPARE_GREATERTHAN: return ">";
            case duckdb::ExpressionType::COMPARE_LESSTHAN: return "<";
            case duckdb::ExpressionType::COMPARE_GREATERTHANOREQUALTO: return ">=";
            case duckdb::ExpressionType::COMPARE_LESSTHANOREQUALTO: return "<=";
            case duckdb::ExpressionType::COMPARE_NOTEQUAL: return "!=";
            default: return "=";
        }
    }
...
private:
    static std::unique_ptr<NativeMaterializedView> create_filter_view(
        ...
        if (def.source_tables.empty()) return nullptr;
        ...
        auto predicate = [resolved_filters](const DuckDBRow& row) -> bool {
            for (const auto& [col_idx, filter] : resolved_filters) {
                if (col_idx >= row.columns.size()) return false;
        ...
        return std::make_unique<NativeFilterView>(
            def.view_name, def.sql, table, schema, predicate);
    }
    ...
    static std::unique_ptr<NativeMaterializedView> create_project_view(
        ...
        for (const auto& col_name : def.project_column_names) {
            ...
            if (idx < source_schema.columns.size()) {
                ...
                result_schema.columns.push_back({col_name, duckdb::LogicalType::VARCHAR});
            }
        }
        ...
        return std::make_unique<NativeProjectView>(
            def.view_name, def.sql, table, result_schema, project);
    }
    ...
    static std::unique_ptr<NativeMaterializedView> create_aggregate_view(
        ...
        if (def.source_tables.empty() || def.aggregates.empty()) return nullptr;
        const std::string& table = def.source_tables[0];
        ...
        size_t value_col_idx = find_column_index(source_schema, agg.value_column_name);
        ...
        result_schema.columns.push_back({agg.function, duckdb::LogicalType::BIGINT});
        ...
        // Value function
        auto value_fn = [value_col_idx](const DuckDBRow& row) -> duckdb::Value {
        ...
        if (agg.function == "COUNT") agg_type = NativeAggregateView::AggType::COUNT;
        else if (agg.function == "AVG") agg_type = NativeAggregateView::AggType::AVG;
        else if (agg.function == "MIN") agg_type = NativeAggregateView::AggType::MIN;
        else if (agg.function == "MAX") agg_type = NativeAggregateView::AggType::MAX;

        return std::make_unique<NativeAggregateView>(
            def.view_name, def.sql, table, result_schema, key_fn, value_fn, agg_type);
    }
    ...
    static std::unique_ptr<NativeMaterializedView> create_join_view(
        ...
        if (!def.join_info.has_value()) return nullptr;
    ...
    static std::unique_ptr<NativeMaterializedView> create_distinct_view(
        ...
        return std::make_unique<NativeDistinctView>(def.view_name, def.sql, table, schema);
    }

    static size_t find_column_index(const TableSchema& schema, const std::string& name) {
