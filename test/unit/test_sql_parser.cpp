...
TEST_CASE("SQL Parser - WHERE clause", "[sql_parser]") {
  ...
  SECTION("Simple equality filter") {
    auto result =
        parser.parse("SELECT * FROM users WHERE age = 25", "filtered_users");
  ...
  SECTION("Greater than filter") {
    auto result =
        parser.parse("SELECT * FROM users WHERE age > 18", "adult_users");
  ...
  SECTION("Less than filter") {
    auto result = parser.parse("SELECT * FROM products WHERE price < 100",
                               "cheap_products");
...
TEST_CASE("SQL Parser - GROUP BY", "[sql_parser]") {
  ...
  SECTION("Simple aggregate with GROUP BY") {
    auto result = parser.parse(
        "SELECT city, COUNT(id) FROM users GROUP BY city", "users_by_city");
...
TEST_CASE("SQL Parser - JOIN", "[sql_parser]") {
  ...
  SECTION("Simple INNER JOIN") {
    auto result = parser.parse("SELECT * FROM orders JOIN customers ON "
    ...
    REQUIRE(result.view_def.join_info->column_pairs.size() == 1);
    REQUIRE(result.view_def.join_info->column_pairs[0].first == "customer_id");
    REQUIRE(result.view_def.join_info->column_pairs[0].second == "id");
...
TEST_CASE("SQL Parser - DISTINCT", "[sql_parser]") {
  ...
  SECTION("SELECT DISTINCT") {
    auto result =
        parser.parse("SELECT DISTINCT city FROM users", "distinct_cities");
  ...
  SECTION("SELECT DISTINCT with multiple columns") {
    auto result = parser.parse("SELECT DISTINCT city, state FROM users",
                               "distinct_locations");
...
TEST_CASE("SQL Parser - View referencing another view", "[sql_parser]") {
  ...
  SECTION("View on top of view") {
    ...
    auto derived_result =
        parser.parse("SELECT name, city FROM adult_users", "adult_user_names");
...
TEST_CASE("SQL Parser - View type determination", "[sql_parser]") {
  ...
  SECTION("JOIN has highest priority") {
    auto result =
        parser.parse("SELECT DISTINCT * FROM orders JOIN customers ON "
                     "orders.customer_id = customers.id WHERE amount > 100",
                     "complex_view");
  ...
  SECTION("AGGREGATE has priority over DISTINCT") {
    auto result =
        parser.parse("SELECT DISTINCT city, COUNT(id) FROM users GROUP BY city",
                     "distinct_agg");
  ...
  SECTION("DISTINCT has priority over FILTER") {
    auto result = parser.parse("SELECT DISTINCT city FROM users WHERE age > 18",
                               "distinct_filter");
  ...
  SECTION("PROJECT type") {
    auto result =
        parser.parse("SELECT id, name, email FROM users", "project_view");
...
TEST_CASE("SQL Parser - Comparison operators", "[sql_parser]") {
  ...
  SECTION("Less than or equal") {
    auto result =
        parser.parse("SELECT * FROM products WHERE price <= 100", "affordable");
  ...
  SECTION("Not equal") {
    auto result = parser.parse("SELECT * FROM users WHERE status != 'inactive'",
                               "active_users");
...
TEST_CASE("SQL Parser - HAVING clause", "[sql_parser][having]") {
  ...
  SECTION("Simple HAVING with COUNT") {
    auto result = parser.parse("SELECT dept, COUNT(id) FROM employees GROUP BY "
                               "dept HAVING COUNT(id) > 10",
                               "large_depts");

    REQUIRE(result.success);
    REQUIRE(result.view_def.type == ParsedViewDef::ViewType::AGGREGATE);
    REQUIRE(result.view_def.having_filters.size() == 1);
    REQUIRE(result.view_def.having_filters[0].ref_type ==
            ParsedViewDef::HavingFilter::AGGREGATE_RESULT);
  ...
  SECTION("HAVING with SUM") {
    auto result = parser.parse("SELECT category, SUM(price) FROM products "
  ...
  SECTION("HAVING with group column reference") {
    auto result = parser.parse("SELECT dept, COUNT(id) FROM employees GROUP BY "
                               "dept HAVING dept = 'Sales'",
                               "sales_dept");

    REQUIRE(result.success);
    REQUIRE(result.view_def.having_filters.size() == 1);
    REQUIRE(result.view_def.having_filters[0].ref_type ==
            ParsedViewDef::HavingFilter::GROUP_COL);
  ...
  SECTION("HAVING with COUNT(*)") {
    auto result = parser.parse("SELECT dept, COUNT(*) FROM employees GROUP BY "
                               "dept HAVING COUNT(*) > 5",
                               "big_depts");
